...
for '_i' from 0 to (count _v)-1 do {
	...
	if (count _pool > 0) then {
		...
		if (isNil {missionNamespace getVariable format ["CTI_SQUAD_%1_%2", _side, _v select _i]}) then {
			...
			if (CTI_Log_Level >= CTI_Log_Debug) then { ["VIO_DEBUG", "FILE: Common\Config\Squads\Squads_Set.sqf", format ["Squad gets combined new pool: <%1>", _pool]] call CTI_CO_FNC_Log };
		} else {
			_new_pool = [];
