...
if (CTI_Log_Level >= CTI_Log_Debug) then {["VIOC_DEBUG", "FILE: common\config\Towns_other_mods.sqf", format["Town Squad preparation - sid: <%1> tag: <%2> ", _sid, _tag]] call CTI_CO_FNC_Log;};
...
if(_loadSabPlanes) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		if (isClass(configFile >> "CfgVehicles" >> format["sab_panther%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_panther%1", _sid],1]];//Mai 1949					Jagdbomber
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_t28trojan%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_t28trojan%1", _sid],1]];//1950						Trainingsflugzeug	
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_mystere%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_mystere%1", _sid],1]];	//1954						Jagdbomber
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_fiat%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_fiat%1", _sid],1]];//August 1956				Erdkampf- und Aufklärungsflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_jaguar%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_jaguar%1", _sid],1]];//1956					Jagdflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_magister%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_magister%1", _sid],1]];//1957						Strahltrainer
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_etendard%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_etendard%1", _sid],1]];//Mai 1958					trägergestütztes Jagdflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_f104%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_f104%1", _sid],1]];//Februar 1958				Abfangjäger
			};
		};
	};
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		if (isClass(configFile >> "CfgVehicles" >> format["sab_mirageiv%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_mirageiv%1", _sid],1]];//Oktober 1964				Strategischer Bomber
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_sf260%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_sf260%1", _sid],1]];//Juli 1964					Trainingsflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_f11tiger%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_f11tiger%1", _sid],1]];//1973					Jagdbomber
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_pc7%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_pc7%1", _sid],1]];//1978						Schulflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_alphajet%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_alphajet%1", _sid],1]];//1979						Leichter Jagdbomber
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_baehawk%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_baehawk%1", _sid],1]];//1974						Erdkampfflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_baehawk%1_2", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_baehawk%1_2", _sid],1]];//1974						Erdkampfflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_mb339%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_mb339%1", _sid],1]];//August 1979				Leichtes Erdkampfflugzeug
			};
		};
	};
	_matrix_cnt = [_matrix_cnt, _matrix_full, _matrix_nation] call CTI_CO_FNC_CheckCountUp;
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		if (isClass(configFile >> "CfgVehicles" >> format["sab_m2000%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_m2000%1", _sid],1]];//Juni 1984					Mehrzweckkampfflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_pc9%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_pc9%1", _sid],1]];//Mai 1984					Schulflugzeug
			};
		};
		if (isClass(configFile >> "CfgVehicles" >> format["sab_t45goshawk%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_t45goshawk%1", _sid],1]];//1991						Trainingsflugzeug
			};
		};
	};
	_matrix_cnt = [_matrix_cnt, _matrix_full, _matrix_nation] call CTI_CO_FNC_CheckCountUp;
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> format["sab_pc21%1", _sid])) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["sab_pc21%", _sid],1]];
			};
		};
	};
};
...
if(CTI_SOV_SIDE == _sideID || CTI_NVA_SIDE == _sideID) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "pook_MIG21_OPFOR")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1pook_MIG21_OPFOR", _sid],1]];
			};	
		};
	};
	_matrix_cnt = [_matrix_cnt, _matrix_full, _matrix_nation] call CTI_CO_FNC_CheckCountUp;
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "pook_SU30_OPFOR")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1pook_SU30_OPFOR", _sid],1]];
				//AIR_FIGHTER append [[format["%1pook_SU30M_OPFOR", _sid],1]];
			};			
		};
...
if(CTI_BW_SIDE == _sideID) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "Tornado_AWS_GER")) then {
			...
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1Tornado_AWS_GER", _sid],1]];
			};
			/*if(CTI_AIR_ADDON >= 2) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= (_level+1)) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "pook_EF2000_BLUFOR")) then {	
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1pook_EF2000_BLUFOR", _sid],1]];
			};		
		};
...
if(CTI_BAF_SIDE == _sideID) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_AV8B_GR9A_2")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1FIR_AV8B_GR9A_2", _sid],1]];
			};
			//if(CTI_AIR_ADDON >= 2) then {
			//	AIR_FIGHTER append [[format["%1FIR_AV8B_GR7A_Lucy", _sid],1]];
			//};	
		};
	};
	if(CTI_ECONOMY_LEVEL_AIR >= 3) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "Tornado_AWS_UK_DS")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1Tornado_AWS_camo_uk", _sid],1]];
			};
			//if(CTI_AIR_ADDON >= 2) then {
			//	AIR_FIGHTER append [[format["%1Tornado_AWS_ADV_uk", _sid],1]];
			//};
			//if(CTI_AIR_ADDON >= 3) then {
			//	AIR_FIGHTER append [[format["%1Tornado_AWS_UK_DS", _sid],1]];
			//	AIR_FIGHTER append [[format["%1Tornado_AWS_UK_617", _sid],1]];
			//};		
		};
	...
	if(CTI_ECONOMY_LEVEL_AIR >= 4) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "EA18G_RAAF") && CTI_US_SIDE != _sideID) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1EA18G_RAAF", _sid],1]];
			};
		};
		//Project Lightning - F-35B Armaverse Version					31. Juli 2015				Mehrzweckkampfflugzeug
...
if(CTI_US_SIDE == _sideID) then {
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_AV8B_VMA223")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1FIR_AV8B_VMA223", _sid],1]];
			};
			/*if(CTI_AIR_ADDON >= 2) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_F14A_Iceman")) then {
			if(CTI_AIR_ADDON >= 0) then {
				//skin pack 1
				AIR_FIGHTER append [[format["%1FIR_F14B_VF102", _sid],1]];
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_F15A_BT")) then {
			if(CTI_AIR_ADDON >= 0) then {
				...
				AIR_FIGHTER append [[format["%1FIR_F15B_5FIS", _sid],1]];
				AIR_FIGHTER append [[format["%1FIR_F15C_AK_CFT", _sid],1]];
				AIR_FIGHTER append [[format["%1FIR_F15D_LN", _sid],1]];
				AIR_FIGHTER append [[format["%1FIR_F15E_Demo", _sid],1]];
				AIR_FIGHTER append [[format["%1FIR_F15SE_CR", _sid],1]];	
			};				
			/*if(CTI_AIR_ADDON >= 2) then {	
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_A10A_MD")) then {
			if(CTI_AIR_ADDON >= 0) then {	
				//skinpack 3
				AIR_FIGHTER append [[format["%1FIR_A10C_MD", _sid],1]];
			};
			/*if(CTI_AIR_ADDON >= 2) then {	
				//skinpack 3
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_F16C_930550_sqd")) then {
			if(CTI_AIR_ADDON >= 0) then {
				//55th USAF
				AIR_FIGHTER append [[format["%1FIR_F16D_900482", _sid],1]];
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_F18C_VMFA251CAG")) then {
			if(CTI_AIR_ADDON >= 0) then {	
				AIR_FIGHTER append [[format["%1FIR_F18C_VMFA251CAG", _sid],1]];
	...
	if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_F22_Blue")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1FIR_F22_Blue", _sid],1]];
			};
			/*if(CTI_AIR_ADDON >= 2) then {
		...
		if (isClass(configFile >> "CfgVehicles" >> "FIR_EA18G_390ECS")) then {
			if(CTI_AIR_ADDON >= 0) then {
				AIR_FIGHTER append [[format["%1FIR_EA18G_390ECS", _sid],1]];
			};
			/*if(CTI_AIR_ADDON >= 2) then {
	...
	case west: {
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_AV8B_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1FIR_AV8B_Blank", _sid],1]];
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_F14A_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {
					...
					AIR_FIGHTER append [[format["%1FIR_F14B_Blank", _sid],1]];
					AIR_FIGHTER append [[format["%1FIR_F14D_Blank", _sid],1]];	
				};
				/*if(CTI_AIR_ADDON >= 2) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_F15C_Blank") && CTI_US_SIDE != _sideID) then {	
				if(CTI_AIR_ADDON >= 1) then {
					...
					AIR_FIGHTER append [[format["%1FIR_F15D_Blank", _sid],1]];
					AIR_FIGHTER append [[format["%1FIR_F15E_Blank", _sid],1]];
					AIR_FIGHTER append [[format["%1FIR_F15SE_Blank", _sid],1]];
				};
				/*if(CTI_AIR_ADDON >= 2) then {	
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_A10A_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1FIR_A10A_Blank", _sid],1]];//white
					AIR_FIGHTER append [[format["%1FIR_A10C_Blank", _sid],1]];
				};
				/*if(CTI_AIR_ADDON >= 2) then {		
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_F16D_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {	
					...
					AIR_FIGHTER append [[format["%1FIR_F16D_Blank", _sid],1]];	
				};
				/*if(CTI_AIR_ADDON >= 2) then {
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "JS_JC_FA18E")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1JS_JC_FA18E", _sid],1]];//1 seat	
					AIR_FIGHTER append [[format["%1JS_JC_FA18F", _sid],1]];//2 seats	
				};
			};
		};
		_matrix_cnt = [_matrix_cnt, _matrix_full, _matrix_nation] call CTI_CO_FNC_CheckCountUp;
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_F2A_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {
					...
					AIR_FIGHTER append [[format["%1FIR_F2B_Blank", _sid],1]];
				};
				/*if(CTI_AIR_ADDON >= 2) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_EA18G_Blank") && CTI_US_SIDE != _sideID) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1FIR_EA18G_Blank", _sid],1]];
				};
				/*if(CTI_AIR_ADDON >= 2) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "AWS_SU34_CDF_BOMBER")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1AWS_SU34_CDF_BOMBER", _sid],1]];
				};
			};
			//Project Lightning - F-35B Armaverse Version					31. Juli 2015				Mehrzweckkampfflugzeug
	...
	case east: {
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "FIR_F14D_ENEMY_TYPE_A")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1FIR_F14D_ENEMY_TYPE_A", _sid],1]];
				};
			};
			//A-10 Warthog												März 1977					Erdkampfflugzeug
			if (isClass(configFile >> "CfgVehicles" >> "FIR_A10C_Enemy_Type1")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1FIR_A10C_Enemy_Type1", _sid],1]];
				};
			};
		};
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "Tornado_AWS_opf")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1Tornado_AWS_opf", _sid],1]];
				};
				/*if(CTI_AIR_ADDON >= 3) then {
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "pook_EF2000_OPFOR")) then {	
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1pook_EF2000_OPFOR", _sid],1]];
				};		
			};
			...
			if (isClass(configFile >> "CfgVehicles" >> "sab_su34_O")) then {
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1sab_su34_O", _sid],1]];
				};	
			};
			//SU-34SM FULLBACK-D AWS										9. Dezember 2011				Jagdbomber
			if (isClass(configFile >> "CfgVehicles" >> "AWS_SU34_genericopfor_BOMBER") && CTI_SOV_SIDE != _sideID) then {
				/*if(CTI_AIR_ADDON >= 1) then {	
					AIR_FIGHTER append [[format["%1AWS_SU34_genericopfor_BOMBER", _sid],1]];
				};
				if(CTI_AIR_ADDON >= 2) then {	
	...
	default {
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "Tornado_AWS_ind")) then {
				if(CTI_AIR_ADDON >= 2) then {
					AIR_FIGHTER append [[format["%1Tornado_AWS_ind", _sid],1]];
		...
		if(CTI_ECONOMY_LEVEL_INFANTRY >= _level) then {
			...
			if (isClass(configFile >> "CfgVehicles" >> "pook_EF2000_INDFOR")) then {	
				if(CTI_AIR_ADDON >= 1) then {
					AIR_FIGHTER append [[format["%1pook_EF2000_INDFOR", _sid],1]];
				};		
			};
		};
		*/
