	...
	if (_side_owned && _order in [CTI_ORDER_CLIENT_TAKEHOLDTOWN_AUTO, CTI_ORDER_CLIENT_TAKEHOLDTOWN]) then {
		if (_town distance _ai < CTI_AI_ORDER_TAKEHOLDTOWNS_RANGE) then {
			...
			while {true} do {
				...
				_lastSV = _town getVariable 'cti_town_SV';
				_currentSV = _town getVariable 'cti_town_SV';
				if ((_currentSV < _lastSV && CTI_P_SideID == (_town getVariable 'cti_town_sideID')) || CTI_P_SideID != (_town getVariable 'cti_town_sideID')) then {
...
CTI_FSM_UpdateClientAI_Order_HoldTownsBase = {
	...
	//--- We patrol!
	_move_defend_last = 0;_move_patrol_reload = true;_move_retake_last = 0;
	...
	while {true} do {
		...
		if !(isNil {_defend getVariable "cti_town_sideID"}) then {
			...
			_camps = _defend getVariable 'cti_town_camps';
			_friendlyCamps = [_defend, CTI_P_SideJoined] call CTI_CO_FNC_GetFriendlyCamps;
			_enemyCamps = _camps - _friendlyCamps;
			if ((_currentSV < _lastSV && CTI_P_SideID == (_defend getVariable 'cti_town_sideID')) || CTI_P_SideID != (_defend getVariable 'cti_town_sideID')) then {
				_action = "defense";
				if (_action != _last_action) then {
					_move_defend_last = -120;
				};
			} else {
				_action = "patrol";
				if (_action != _last_action) then {
			...
			if (count _camps != count _friendlyCamps) then {
				_action = "retake";
				if (_action != _last_action) then {
					_move_retake_last = -120;
				};
			};
		};
		...
		switch (_action) do {
			case "patrol": {
				if (_move_patrol_reload || unitReady _ai && random 100 > (45 + (random 15))) then {
					if (_move_patrol_reload) then {
						_move_patrol_reload = false;
					};
					_move_to = _patrol_area select floor(random count _patrol_area);
					vehicle _ai doMove _move_to;
					((_ai getVariable "cti_ai_formation") - [_ai]) doFollow _ai;
				}
			};
			
			case "defense": {
				if (unitReady _ai && time - _move_defend_last > 65) then {
					_move_defend_last = time;
					vehicle _ai doMove ([_defend, 10, 50] call CTI_CO_FNC_GetRandomPosition);
					((_ai getVariable "cti_ai_formation") - [_ai]) doFollow _ai;
				};
			};
			
			case "retake": {
				if (unitReady _ai && time - _move_retake_last > 65) then {
					_move_retake_last = time;
					_camps = _defend getVariable 'cti_town_camps';
					_friendlyCamps = [_defend, CTI_P_SideJoined] call CTI_CO_FNC_GetFriendlyCamps;
					_enemyCamps = _camps - _friendlyCamps;
					_retake = _enemyCamps select floor (random count _enemyCamps);
					vehicle _ai doMove ([_retake, 5, 10] call CTI_CO_FNC_GetRandomPosition);
					((_ai getVariable "cti_ai_formation") - [_ai]) doFollow _ai;
				};
			};
		};
