...
CTI_FSM_UpdateAI_Order_TakeTown = {
	...
	if (_side_owned && _order in [CTI_ORDER_TAKEHOLDTOWN_AUTO, CTI_ORDER_TAKEHOLDTOWN]) then {
		if (_town distance leader _group < CTI_AI_ORDER_TAKEHOLDTOWNS_RANGE) then {
			//--- We patrol!
			_move_defend_last = 0;_move_patrol_reload = true;_move_retake_reload = 0;
			...
			while {true} do {
				...
				_friendlyCamps = [_town, CTI_P_SideJoined] call CTI_CO_FNC_GetFriendlyCamps;
				_enemyCamps = _camps - _friendlyCamps;
				if ((_currentSV < _lastSV && _sideID == (_town getVariable 'cti_town_sideID')) || _sideID != (_town getVariable 'cti_town_sideID')) then {
					_action = "defense";
				...
				switch (_action) do {
					...
					case "retake": {
						if (unitReady leader _group && time - _move_retake_last > 65) then {
							_move_retake_last = time;
							_camps = _town getVariable 'cti_town_camps';
							_friendlyCamps = [_town, _side] call CTI_CO_FNC_GetFriendlyCamps;
							_enemyCamps = _camps - _friendlyCamps;
							_retake = _enemyCamps select floor (random count _enemyCamps);
							_group move ([_retake, 5, 10] call CTI_CO_FNC_GetRandomPosition);
						};
					};
				};
