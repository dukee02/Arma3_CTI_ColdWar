...
while { alive _factory } do {
	...
	_req_unit_count = _request select 8;
...
//todo req_toai for indep salvager from client!
	if (_req_toai) then { //--- Send a purchase request to the client if the target is not an AI.
		...
		[_req_seed, _req_classname, _req_buyer, _req_target, _factory, _req_side, _req_veh_infos] spawn CTI_SE_FNC_HandleAIPurchase;
		[["CLIENT", leader _req_buyer], "Client_PurchaseQueueRelease", [_factory, _req_unit_count]] call CTI_CO_FNC_NetSend;
	} else {
		//--- Target != source? notify the source that his remote req is being handled.
		if (typeName _req_target != "SIDE") then {
			if (_req_buyer != _req_target) then {[["CLIENT", leader _req_buyer], "Client_PurchaseDelegationStart", [_req_seed, _req_classname, _req_target, _factory]] call CTI_CO_FNC_NetSend};
			
			[["CLIENT", leader _req_target], "Client_OnPurchaseOrderReceived", [_req_seed, _req_classname, _req_buyer, _factory, _req_veh_infos, _req_unit_count]] call CTI_CO_FNC_NetSend;
